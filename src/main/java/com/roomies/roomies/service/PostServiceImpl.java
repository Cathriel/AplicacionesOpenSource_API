...
public class PostServiceImpl implements PostService {
    ...
    public Post updatePost(Long postId, Post postRequest) {
        ...
        post.setAddress(postRequest.getAddress())
                ...
                .setPicture(postRequest.getPicture())
    ...
    @Override
    public Page<Post> getAllPostsByLandlordId(Long LandlordId,Pageable pageable) {
        return landlordRepository.findById(LandlordId).map(
                landlord -> {
                    List<Post> postList = landlord.getPosts();
                    int postsCount = postList.size();
                    return new PageImpl<>(postList,pageable,postsCount);
                }
